...
# data_dir = '/home/rb/data/airsim_datasets/soccer_close_50k'
data_dir = '/home/rb/data/airsim_datasets/soccer_cal_50k_new'
# weights_path = '/home/rb/data/model_outputs/cmvae_con/cmvae_model_40.ckpt'
weights_path = '/home/rb/data/model_outputs/cmvae_con_cal_new/cmvae_model_20.ckpt'
...
output_dir = '/home/rb/data/model_outputs/cmvae_real_cal_new_2'
batch_size = 32
epochs = 10000
n_z = 10
img_res = 64
...
def train(img_gt, gate_gt, img_real, epoch, mode):
    with tf.GradientTape() as tape:
        _, gate_recon, means, stddev, z = model(img_gt, mode=2)
        img_recon_real, _, _, _, _ = model(img_real, mode)
        _, img_loss_real, gate_loss, kl_loss = compute_loss_unsupervised(None, img_real, gate_gt, None, img_recon_real, gate_recon, means, stddev, mode)
        img_loss_real = tf.reduce_mean(img_loss_real)
        gate_loss = tf.reduce_mean(gate_loss)
        beta, _, w_img_real, w_gate = regulate_weights(epoch)
        # weighted_loss_img = calc_weighted_loss_img(img_recon, img_gt)
        if mode == 0:
            total_loss = w_img_real*img_loss_real + w_gate*gate_loss + beta*kl_loss
