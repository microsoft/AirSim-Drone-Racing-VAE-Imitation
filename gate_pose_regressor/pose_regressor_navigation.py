...
if __name__ == "__main__":
    ...
    time.sleep(0.2)
    time.sleep(0.05)
    takeoff_pose = airsim.Pose(airsim.Vector3r(0, 0, 10), airsim.Quaternionr(0, 0, 0, 1))
    direction_vector = racing_utils.geom_utils.get_gate_facing_vector_from_quaternion(takeoff_pose.orientation, scale=1.0)
    ...
    client.moveOnSplineVelConstraintsAsync([takeoff_pose.position], [airsim.Vector3r(1,0,0)], vel_max=vel_max, acc_max=acc_max, vehicle_name=drone_name, viz_traj=True).join()
    ...
    while True:
        ...
        #                                       vel_max=vel_max, acc_max=acc_max, vehicle_name=drone_name, viz_traj=True)
        client.moveOnSplineAsync([gate_pose.position], add_curr_odom_position_constraint=True, add_curr_odom_velocity_constraint=True, vel_max=vel_max, acc_max=acc_max, vehicle_name=drone_name, viz_traj=True)
